...
// 3. 'static 生命周期：特殊的生命周期，穿通整个程序运行期。
...
impl<'a> ImportantExcerpt<'a> {
    ...
    // 省略规则 2: 如果只有一个输入生命周期，它会被赋予所有输出生命周期。
    // 省略规则 3: 如果有 &self 或 &mut self，那么 self 的生命周期会赋予所有输出生命周期。
    ...
    // 也就是返回值的生命周期来自结构体实例本身（或者说结构体引用的数据）
...
fn example3_static_lifetime() {
    ...
    // 1. 字符串字面值拥有 'static 生命周期，因为它们被硬码在程序二进制文件中
...
// ============================================================================ 
// 示例 4: 综合应用 - 模板类、Trait Bound 和 生命周期
// ============================================================================ 
use std::fmt::Display;
